...
*/
#include <iostream>
//Aqui você criou um alias, um apelido para struct Livro.
//Este apelido se chama RegistroLivro
...
//agora RegistroLivro é uma variável!!! criada via
//forma compactada de criação de variáveis do tipo struct

int main()
{	
	//Aqui é criada uma variável do tipo struct Livro
    //mas sem usar o alias o apelido criado via typedef
	//Agora um vetor de Structs
	struct Livro Aluno01[5];
	//Aqui criamos uma variável do tipo RegistroLivro
	//Porém RegistroLivro é apenas um apelido uma alias
    //para struct Livro
	//O pré-processador vai trocar este nome por struct
    //Livro antes da compilação, simples assim...
	//Porém muitos usam para simplicar o código e digitar menos
	RegistroLivro Aluno02;
	//Criamos um ponteiro capaz de apontar para uma variável do tipo
    //RegistroLivro que nada mais é que um alias de struct livro
